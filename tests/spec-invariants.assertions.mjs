...
function run() {
  ...
  assert.equal(LESSONS_SPEC.lessonsPerPeriod, 5);
  assert.equal(LESSONS_SPEC.lessonsTotal, 15);
  assert.equal(LESSONS_SPEC.lessonMax, 10);
  assert.equal(LESSONS_SPEC.trainingMax, 7);
  assert.equal(LESSONS_SPEC.productionMax, 3);
  assert.equal(LESSONS_SPEC.periodMax, 50);
  ...
  periods.forEach((period) => {
    ...
    assert.equal(periodLessons.length, 5);
    assert.equal(period.maxScore, 50);
  });
